            ...
            return (
              <Link href={item.link} key={i} passHref>
                <motion.div
                  ...
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  className="tracking-wider cursor-pointer text-gray-50"
                >
                  {item.title}
                </motion.div>
              </Link>
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          animate="shown"
          exit="exit"
          className={` fixed top-0 right-0 z-50 w-80 h-screen px-10 pt-10 space-y-3 bg-blue-primary/80 backdrop-blur-lg`}
        >
